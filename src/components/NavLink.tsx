      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-xl">
            <span className="text-[#C9A84C] text-xs font-bold tracking-[0.3em] uppercase mb-4 block">Curated Selection</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="lg:col-span-7 group relative aspect-[4/5] md:aspect-square lg:aspect-auto lg:h-[700px] overflow-hidden bg-gray-200 cursor-pointer"
            onClick={() => openWhatsApp(favourites[0].name, favourites[0].price)}
          >
            <img src={favourites[0].image} alt={favourites[0].name} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity" />
            <div className="absolute top-8 left-8">
              <span className="bg-white/90 backdrop-blur-md px-4 py-1 text-[10px] font-bold tracking-[0.2em] uppercase shadow-sm">{favourites[0].tag}</span>
            </div>
            <div className="absolute bottom-10 left-10 text-white">
              ...
              <button
                className="flex items-center gap-2 bg-white text-black px-8 py-4 text-xs font-bold tracking-[0.2em] uppercase hover:bg-[#C9A84C] hover:text-white transition-colors"
                onClick={(e) => { e.stopPropagation(); openWhatsApp(favourites[0].name, favourites[0].price); }}
              >
          ...
          <div className="lg:col-span-5 flex flex-col justify-between gap-8">
            {favourites.slice(1).map((product, i) => (
              <motion.div
                ...
                className="group flex gap-6 items-center p-4 hover:bg-white hover:shadow-xl transition-all duration-500 rounded-sm cursor-pointer"
              ...
              >
                <div className="relative w-32 h-32 md:w-40 md:h-40 shrink-0 overflow-hidden bg-white shadow-inner">
                  ...
                  <button onClick={(e) => toggleWishlist(e, i + 1)} className="absolute top-3 right-3 p-1.5 rounded-full bg-white/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
                    <Heart className={`w-4 h-4 transition-colors ${wishlisted[i + 1] ? "fill-red-500 stroke-red-500" : "stroke-gray-600"}`} />
                  </button>
                </div>
                <div className="flex flex-col flex-grow">
                  <span className="text-[9px] tracking-[0.2em] text-[#C9A84C] font-bold uppercase mb-2">{product.tag}</span>
                  <h4 className="text-lg font-medium text-gray-900 mb-1 group-hover:text-[#C9A84C] transition-colors">{product.name}</h4>
                  <p className="text-sm text-gray-500 font-light mb-4">{product.price}</p>
                  <button
                    className="flex items-center gap-2 w-fit bg-[#1a3a3a] text-white text-[10px] tracking-[0.2em] font-bold px-5 py-3 hover:bg-black transition-all transform group-hover:translate-y-0 translate-y-2 opacity-0 group-hover:opacity-100 duration-300"
